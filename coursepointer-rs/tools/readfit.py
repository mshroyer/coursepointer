...
def show_frames(path: str) -> None:
    with fitdecode.FitReader(path, processor=fitdecode.StandardUnitsDataProcessor(), keep_raw_chunks=True) as reader:
        for frame in reader:
            if frame.frame_type == fitdecode.FIT_FRAME_DATA:
                ...
                for field_def in frame.field_defs:
                    print(f"- Field def: {field_def.name} ({field_def.def_num}) = {field_def.type.name} ({field_def.base_type.identifier}) size {field_def.size}")
...
def main() -> None:
    ...
    parser_show_messages = subparsers.add_parser("show-messages", help="Show messages as extracted by the Garmin SDK")
